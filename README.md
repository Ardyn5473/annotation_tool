...
### データ分析機能
アノテーションデータの統計分析と可視化を行う機能です。「運転アノテーション分布」の横にある「分析」ボタンから起動できます。

#### 統計・分布表示
- **統計テーブル**: 各センサー値（angle, throttle, speed等）の平均、標準偏差、最小、最大、中央値を一覧表示
- **分布グラフ**: AngleとThrottleの値の分布をヒストグラムで可視化

#### 時系列グラフ
- **表示モード切替**: 3つの表示形式をラジオボタンで選択
  - **生データ**: 元のデータをそのまま線グラフで表示
  - **移動平均**: 指定した窓サイズ（デフォルト50）で移動平均を計算して表示
  - **区間平均**: 指定した区間（デフォルト200インデックス）ごとの平均値をバーチャートで表示
- **表示項目選択**: angle, throttle以外にもcatalogファイルから読み込んだセンサーデータ（例：imu/acl_x）を選択可能
- **表示範囲調整**: インデックスの表示範囲を指定してズーム表示
- **現在位置連動**: メインウィンドウでの画像移動と連動して現在位置マーカーが移動
- **グラフクリックでジャンプ**: 時系列グラフをクリックすると該当インデックスの画像にジャンプ
