...
def sanitize(name: str) -> str:
    """フォルダ名に安全な文字だけ残す"""
    name = name.strip()
    return re.sub(r"[^0-9A-Za-z_.-]+", "_", name) or "layer"

def find_module_by_name(model:nn.Module, dotted:str):
    m = model
    for p in dotted.split("."):
        if not hasattr(m, p):
            raise ValueError(f"layer '{dotted}' が見つからない: '{p}' がありません")
        m = getattr(m, p)
    return m

def coerce_to_conv_layer(model: nn.Module, target_name: str) -> tuple[nn.Module, str]:
    """
    指定レイヤの出力が4D(NCHW)にならない場合、同モジュール直前のConv2d、さらに遡って最後のConv2dに付け替える
    """
    # まず素直にその名前
    layer = find_module_by_name(model, target_name)

    # 出力次元を軽くテスト
    try:
        with torch.no_grad():
            # ざっくり224正方形でサンプル（前処理のsizeに依存するなら呼び出し側で合わせる）
            x = torch.zeros(1,3,224,224)
            def _capture(_m, _i, o):
                nonlocal out_shape
                out_shape = tuple(o.shape) if hasattr(o, "shape") else None
            out_shape = None
            h = layer.register_forward_hook(_capture)
            _ = model(x)
            h.remove()
        if out_shape and len(out_shape) == 4:
            return layer, target_name   # NCHWならOK
    except Exception:
        pass
    ...
    for name, mod in model.named_modules():
        if isinstance(mod, nn.Conv2d):
            last_conv_name, last_conv = name, mod

    # dottedの直前っぽいところから逆順で探す
    prefix = target_name.rsplit(".", 1)[0] if "." in target_name else ""
    fallback = last_conv_name
    for name, mod in model.named_modules():
        if isinstance(mod, nn.Conv2d):
    ...
    if fallback:
        return find_module_by_name(model, fallback), fallback

    # どうしても見つからなければ元のまま返す
    return layer, target_name
...
class GhostNet100Model(TIMMBasedModel):
    """TIMMベースのGhostNet 100モデル"""
    def __init__(self, pretrained=True):
        super(GhostNet100Model, self).__init__(
            name="ghostnet_100",
            timm_model_name="ghostnet_100",
            pretrained=pretrained
        )
...
class ShuffleNetV2_x05Model(BaseModel):
    """torchvision版 ShuffleNetV2 x0.5 を使う"""
    def __init__(self, pretrained=True, num_outputs=2):
        super().__init__(name="shufflenetv2_x0_5")
        weights = tvm.ShuffleNet_V2_X0_5_Weights.DEFAULT if pretrained else None
        m = tvm.shufflenet_v2_x0_5(weights=weights)

        in_f = m.fc.in_features
        m.fc = nn.Identity()              # ここがポイント：特徴量を直接受け取る
        self.base_model = m
        self.regressor = nn.Linear(in_f, num_outputs)

    def forward(self, x):
        feats = self.base_model(x)        # [N, in_f]
        out = self.regressor(feats)       # [N, 2]（angle, throttle）
        return out
